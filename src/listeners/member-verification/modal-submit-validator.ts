...
export class InteractionCreateListener extends Listener<
  ...
  public override async run(interaction: Interaction) {
    ...
    if (program.ids.length) {
      ...
      if (!response.ok) {
        await this.sendError(
          interaction,
          "Failed to obtain team information from VEX Events",
        );
        throw new Error(
          `Failed to fetch teams from VEX Events: ${response.status}`,
        );
      }
    ...
    const roles = [verifiedRole, program.role, locationRole];
